...
# ]
# ///
"""SQLite → PostgreSQL one-shot data migration (review-first).

기본은 --dry-run 입니다. 실제 쓰기는 --execute 를 명시해야 합니다.
...
사용 예:
  ...
  uv run scripts/migrate_sqlite_to_postgres.py \\
    --sqlite data/app.db \\
    --database-url 'postgresql://ax:secret@localhost:5432/ax_platform'

  # 실제 이관 (대상 PG에 schema.postgres.sql 적용 후 권장)
  uv run scripts/migrate_sqlite_to_postgres.py \\
    --sqlite data/app.db \\
...
주의:
  - 운영 전 반드시 dry-run 로그를 검토하세요.
  - 대상 PG가 비어 있지 않으면 --truncate-target 없이는 충돌할 수 있습니다.
  - 동적 inventory 테이블은 컬럼을 읽어 PG에 CREATE 후 COPY 합니다.
  - k8s_cluster 가 있으면 infra_cluster 로 매핑합니다.
"""
...
def _create_inventory_table(
    ...
    for name, decl in col_types:
        ...
        if name == "idx":
            cols_sql.append(f"{_quote_ident(name)} {pg_type} PRIMARY KEY")
        else:
    ...
    *,
    ...
    if dry_run:
        ...
        if import_mynote_files:
            n = _import_mynote_files(
                sqlite_conn, None, project_root=project_root, dry_run=True
            )
            print(f"[dry-run] would import ~{n} mynote file(s) into mynote_contents")
        sqlite_conn.close()
        return 0
    ...
    try:
        ...
        for plan in plans:
            ...
            if plan.kind == "inventory":
                cols = _create_inventory_table(pg_conn, sqlite_conn, plan.sqlite_name)
            else:
                cols = list(plan.columns)
                # infra_cluster may need extra columns in insert payload
            ...
            if truncate_target:
                try:
                    ...
                    print(f"truncated {plan.postgres_name}")
            ...
            # Filter columns to those that exist on PG table when possible
            n = _insert_rows(
                pg_conn,
                plan.postgres_name,
                cols,
        ...
        if import_mynote_files:
            ...
            pg_conn.commit()
            print(f"imported mynote files → mynote_contents: {n}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--import-mynote-files",
        action="store_true",
        help="Copy data/mynotes/*.md into mynote_contents",
    )
    parser.add_argument(
        "--schema",
        type=Path,
