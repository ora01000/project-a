...
## 2026-08-19 — FossFLOW 다이어그램·작업 처리 (`260819` / `pg260819`)

### FossFLOW
- 에이전트 compact JSON(`"f": "compact"`)을 Isoflow 편집기 없이 **이미지(SVG)로 렌더링**
- 아이소메트릭 그리드, A* 연결선·화살표, 노드 라벨(이름·설명), 호버 툴팁
- 확대/축소·마우스 드래그로 뷰포트 밖 영역 이동, PNG 다운로드(그리드·라인 포함)
- 작업 노트 **FossFLOW** 탭(관리자), 통합 채팅, 나의 노트 미리보기, 나의 작업 결과에 동일 미리보기 적용
- 노트/작업 리포트 **메일** 본문에 compact JSON을 PNG(또는 SVG)로 임베딩

### 작업 처리
- mock 모드에서 승인 작업(status_code=2)이 helpdesk `agentruntime` 부재로 건너뛰어지던 문제 수정 — 로컬 `helpdesk`로 위임

### 인프라·권한 (1.7 이후)
- 인프라 형상: 전체 목록 + 행 선택 상세 분할, `infra_cluster.display_name` 표시
- 갭분석: 관리자 전용, 클러스터 비교 범위 3세대, LLM을 게이트웨이(`PRIVATE_LLM_API_KEY`)·Bifrost 모델 ID로 전환
- 역할 `infraadmin`(2): 일반 user와 동일하되 IP 마스킹 없음
- 작업 검토·나의 검토작업 탭 신규 job 🆕 배지, 세션 연장 확인 다이얼로그 제거
- Whatap 구독자 자동 메일을 렌더링된 HTML/plain 폴백에 맞춤

### 릴리즈
- About: 버전 **1.8**, 릴리즈 **260819**
- Docker 이미지 태그: `pg260819` (`linux/amd64`)
