...
출처: git 커밋, `ADDITIONAL_PLAN.md`, 이후 미커밋 반영분(2026-07-15~16).
...
## 2026-07-10 — 로그인·에이전트 DB·작업 프로세스
...
- 작업 프로세스·대시보드·에이전트 운영 기능 보강 (커밋 `260712`)

---

## 2026-07-13 — 중간 업데이트 (`260713`)

- 시스템 에이전트·알림·헬스체크·인벤토리 등 운영 기능 확장 (커밋 `260713`)
- Whatap 이벤트 수신 시스템 에이전트, Chroma 기반 인벤토리 에이전트
- 에이전트 working/idle/error 상태 표시, 로그 탭, 주기적 헬스체크

---

## 2026-07-14~15 — UI·작업·인증·테스트 (`260714` 커밋, 계획 반영)
...
## 2026-07-15 — 헬프데스크·계획/수행 고도화 (`260715`, 이후 작업 포함)
...
## 2026-07-16 — 할당·권한·디버깅 UI (`260716`)
...
### 사용자별 에이전트 할당
- `users.agents` 컬럼(`VARCHAR(200)`, 콤마 구분 `agent_id`) 추가·마이그레이션
- 사용자 관리 → 에이전트 할당 팝업
  - 좌: 일반 에이전트 레이블(다중 선택, Ctrl/Cmd+A 전체·Esc 해제)
  - 우: 사용자 표(조직/이름/역할/할당 에이전트)
  - 드래그 앤 드롭으로 할당, 저장/닫기
- 로그인·대시보드 동기화로 할당 반영, 채팅 API 미할당 호출 403
...
- 2026-07-15 이후 일부 항목은 작성 시점 기준 워킹 트리 반영분이며, 원격 커밋과 다를 수 있습니다.
