...
# 실행중인 workflow 에서 UI 개선
- 실행중인 workflow 에서는 모든 편집이 불가하다. 삭제 "X" 버튼, work_node "+" 버튼은 숨긴다.
- 실행중인 work_node 의 작업을 중지할 수 있다. "X" 버튼 대신 "중지" 버튼을 배치한다.
- 중지 시 해당 워크노드는 실패로 처리하고 전체 작업 워크플로우도 실패로 끝나고 종료 처리한다.
  - 중지 버튼 클릭시 확인 창을 띄워 확인한다.

- 다이어그램 생성 프롬프트 패널은 "새로운 작업 워크플로우" 생성 화면에서만 출력된다. 왼쪽 패널에서 이미 생성된 목록을 선택할 경우 다이어그램 생성 프롬프트(양식 선택 포함) 영역은 나타나지 않는다.

# 로그 로테이션 및 보관
- agents/{에이전트명}.log 를 로테이션 한다.
  - 로테이션 주기 : 1일
  - 이전 로그 : 로테이션 시점의 일자를 '_YYYYMMDDHHMISS' 로 파일명에 붙여서 보관하고 최대 3개 보관하며 나머지는 모두 gz 으로 아카이빙한다. 아카이빙은 3개까지 보관
- user_comm_logs/{userid}/{일자}.json
  - 대화기록은 사용자 당 3개 보관, 나머지는 모두 gz 아카이빙한다. 아카이빙은 3개까지 보관
- 보관 개수에 대한 환경변수, default 값
  - default 는 모두 3개이고, 각 개수정보는 환경변수로 관리한다.

# 작업 워크플로우 대화 로그 별도 처리/표현
작업 워크플로우와 work_node의 에이전트 통신 기록을 기존 대시보드 > 대화형 터미널, 대시보드 > 대화로그 와 다른 방식으로 처리하고자 한다.
- WORKFLOW_AGENT 통신기록, work_node 의 에이전트 통신기록(이제 남김) 은 아래 경로 아래 둔다. 대화형 터미널에서는 더이상 이 로그는 보여주지 않는다.
  - logs/workflow/{users.userid}/{workflow.uuid}/workflow.log
  - logs/workflow/{users.userid}/{work_node.uuid}/work_node.log
- 로그 로테이션 정책은 다른 로그와 동일하다.

# work_node 실패 오류 메시지 표시 위치
현재 work_node 에서 실패 응답을 받은 경우 에러 메시지를 왼쪽 "작업워크플로우 목록" 패널의 상단에 출력하고 있다. 실패 응답 메시지는 work_node 로그로 출력하는지를 확인해 달라

분석된 각각의 상황에 대해 다음과 같이 동작하도록 개선
- 에이전트가 응답을 반환한 뒤 구조화 검증 실패(success=false 등) : 기존과 동일
- invoke 자체 실패/빈응답/중지 : workflow 로그에 남겨야 할 것 같다
- DB last_fail_reason : 로그에도 동일하게 기록

- 로그 UI 표시
  - 작업 결과 탭에서 결과 내용과 별도의 패널로 로그를 출력

# 대시보드 화면 레이아웃 수정
- 대화형 터미널을 에이전트 노드 목록 처럼 폴딩할 수 있게 한다. 대화형 터미널은 오른쪽으로 접히게 한다.
- 상세정보 패널, 대화형 터미널은 최초 로딩시 접힌 상태로 로딩된다

# 작업워크플로우 화면 미선택시 공백에 대한 처리
- 작업워크플로우 화면 전환시 첫화면에서 workflow 목록이 선택되지 않았거나, 목록이 없을때 오른쪽 편집 화면이 공백이다. 다음 위치의 MD 파일을 렌더링 해서 출력하고자 한다.
  - docs/workflow_front/workflow_front.md
  - workflow_front.md 파일을 생성하고 다음 내용을 넣는다(한글 그대로 작성). 섹션이나 단락의 배치는 다듬어 달라. 특히 {생성} 영역에 각 섹션에 맞는 내용을 간략하게 요약해서 삽입
'''
# 작업 워크플로우
작업 워크플로우는 에이전트 작업을 순차적으로 구성하고 관리자 승인 절차를 구성하여 작업계획을 만들고 수행, 이력을 관리할 수 있습니다.
...
# WORKFLOW_AGENT에서 워크플로우 생성시 CRUD 판단
- WORKFLOW_AGENT에서 응답시 각 단계별 작업의 CRUD 를 판단한다(결재 work_node 제외). 에이전트의 응답 json 에 이를 식별해서 삽입하도록 시스템 프롬프트를 개선한다.
  - 추가 json 컬럼
    - "crud" : [ "c" | "r" | "u" | "d" ] 
  - work_node 테이블에 컬럼 추가
    - crud char
- 작업 워크플로우 목록의 workflow 카드에 이를 표시한다. workflow 에 소속된 work_node 의 crud 를 판단하고 조합한다.
  - C : "생성"
  - R : "읽기"
  - U : "갱신"
  - D : "삭제"
...
# 처리시간이 오래 걸리는 작업 시 아이콘
- 처리가 진행중임을 표시하는 돌아가는 아이콘으로 현재 동작중임을 애니메이션으로 시각적으로 표시한다.
  - 대상
    - 다이어그램 생성 프롬프트 패널의 생성 버튼
    - AI 갭분석 버튼

# 작업 워크플로우 목록에 표시되는 결과 문구
- 현재 작업 워크플로우 목록의 상단에 출력된다. 이를 목록 가장 아래 메시지를 출력하는 패널 박스를 만들고 거기에 출력한다.
- 고정 패널에서 아래로 접히는 폴딩 패널로 변경한다. 출력 결과가 있으면 자동으로 unfold 되며 사용자가 확인 후 닫으면 다시 아래로 접힌다. 패널라인은 보이지 않도록 한다
  - 패널의 세로 길이를 출력된 메시지 길에에 맞춰 생성한다.

# 진행중인 워크플로우의 작업 결과
- 진행중일 때 작업 결과가 실패로 나온다. workflow 가 진행중, 실패 가 구분이 가능하다면 "진행중" 으로 표시한다.

# 진행중인 work_node 의 작업 결과
- work_node 작업이 진행중일 때 작업 결과가 실패로 나온다. work_node 가 진행중일 때 실패가 구분이 가능하면 "진행중" 으로 표시한다.

# 다이어그램 템플릿 양식 작성기
- docs/workflow_template 를 런타임에 수정할 수 있도록 경로를 변경한다 -> {UPLOAD_HOME}/workflow_template (http 모드는 서버에서 서버관리자가 수작업으로 변경한다)
- "새로운 템플릿" 버튼 -> "작업 워크플로우 목록" 패널의 가장 하단에 "새로운 작업 워크플로우" 와 동일한 스타일로 작성한다. 버튼 icon asset은 구별되게 한다.
  - users.role = 0 | 2 | 100 인 사용자만 권한이 부여된다. 다른 사용자는 보이지 않는다.
  - 버튼 클릭시 오른쪽 워크플로우 생성 및 편집 패널 위치에 "새로운 템플릿" 작성 패널이 표시된다.
- 템플릿 관리 테이블을 추가한다.
  - workflow_template
    - idx int primary key
...
- 새로운 템플릿 작성 패널은 다음 정보를 받는다.
  - 양식 이름 : 입력시 기존 템플릿 이름에 동일한 이름이 있는지를 체크한다. 성능 최적화를 위해 workflow_template.template_name 은 redis에서 캐시로 저장하고 비교한다. 파일명에 사용할 수 없는 문자열은 사용하지 못하도록 체크한다(양식명이 파일명이 된다).
  - 로컬 양식파일 참조 : 로컬에서 읽어서 양식 편집 화면에 출력된다. 
  - 기존 양식파일 참조 : 이미 등록된 양식을 읽어 편집 양식 편집 화면에 출력한다.
  - 저장될 파일이름 : 로컬 양식파일에서 참조한 경우 해당 파일명을 쓴다. 양식 이름과 마찬가지로 workflow_template.template_filename 은 redis에서 캐시로 저장하고 비교한다. 입력시 기존 템플릿 파일명이 동일한게 있는지 체크한다.
  - 양식 편집 : 텍스트 영역에서 직접 편집이 가능하다
    - 완쪽에는 에디터 라인이 출력되도록 한다.
    - 편집기의 기본 파일 양식은 MarkDown 이다(고정)
  - 저장하면 양식 편집의 텍스트 영역 내용을 입력된 파일명으로 저장한다.
- 새로운 작업 워크플로우의 다이어그램 생성 프롬프트에서 양식 선택 박스는 이제 workflow_templat.template_name 을 참조한다.
  - 다이어그램 생성 프롬프트에서 양식을 선택하면 {UPLOAD_HOME}/workflow_template/{workflow_template.template_filename} 의 파일을 읽어 온다.

- 기존 템플릿 수정 버튼을 생성한다. 이름 "템플릿 수정", 새로운 템플릿 버튼 위에 배치한다. 저장하면 기존의 파일 내용이 변경된다.
  - 오른쪽 템플릿 수정 패널을 구성한다.
    - 기존 양식파일 선택
    - 양식 이름 -> 변경 가능하다.
    - 저장될 파일이름 -> 변경 가능하다.
...
- 새로운 템플릿의 저장 버튼, 기존 템플릿 수정의 저장 버튼 클릭시 모두 확인 하고 저장한다.

# 마당 인증 방식 변경
- madang 인증(http모드) 에서 인증 프로바이더 정책에 의해 다음과 같이 변경되었다.
...
# 메일 처리
- @docs/skill/job_scope.md  에 대한 보강을 한다.
  1. Criteria for decition_type = 11 에 대해 다음 토픽을 추가한다.
  - 단순 조회성으로 인벤토리(INFRA_SEARCH_AGENT)에 위임할 작업을 토픽에 추가
    - IP, 호스트명, 담당자, 운영자 등 인벤토리 테이블에서 조회를 통해 답변이 가능한 질의
    - 작업 타입 CUD 가 포함되지 않는 질의
    - 이 질의는 decision_type = 10 이다
- 메일 수신 처리시, Office 문서, 이미지 파일은 처리하지 않는다. 저장도 하지 않는다. 또한 job_decision에서 고려하지 않는다.
- 메일 수신시 body_text 처리시 개행문자가 사라지면서 다음 라인이 이전라인의 끝에 붙어서 처리되는 현상이 있다. 문맥의 혼동이 없도록 개행문자를 유지
- 처리 결과를 메일로 회신할 때 plain text 로 나간다. md 로 렌더링 해서 발송한다
- 여기서 발송된 메일은 모두 본문 가장자리에 패널라인이 있다. 패널라인을 삭제
- 처리 결과(보완 요청서, 작업 결과 처리서)를 받은 메일의 회신으로 처리한다.
  - 제목에는 "RE:" 를 앞에 붙여서 요청자가 보낸 메일의 회신임을 밝힌다.
  - 회신되는 메일에는 보내는 본문 아래 원본 수신 메일 내용이 포함된다(회신 처리시 자동으로 그렇게 되면 로직으로 처리할 필요 없다)
- job_decision 결과 read only 작업인 경우 자동승인 로직을 넣고자 한다.
  - job_kind 가 read 인 경우 status_code 를 2 로 처리
  - approver 에는 자동승인 식별용 auto_approve 를 넣는다
