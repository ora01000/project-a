...
- 추가 테이블
  1. work_node
    ...
    - user_prompt text <- 사용자의 지시사항
    - agent_response text <- 에이전트의 결과
...
- 전체 화면의 패널 구조가 신규로 생성되고 다음과 같이 배열된다.
  - left : "워크플로우 목록 패널"
    ...
    - workflow 목록 패널은 workflow 테이블의 내용을 노드 형태로 출력한다. workflow_name 만 표시
    - 패널의 상단에는 가로 100%를 차지하는 "새로운 워크플로우" 생성 버튼을 배치한다.
  - center : "워크플로우 디자인 패널"
    - "새로운 워크플로우" 버튼을 클릭시 워크플로우를 생성할 수 있는 화면이 표시된다.
      - 워크플로우 편집모드
      - 워크플로우 작성은 GUI로 작성한다.
        1. 워크플로우 명 : 최상단에 텍스트필드로 입력 받는다(현재와 동일)
        2. 시작(S) 노드만 있다. 시작 노드 오른쪽에는 + 버튼이 동그라미로 표시된다. + 버튼 을 클릭하면 오른쪽에 워크 노드 다이어그램이 생성되고 화살표로 연결된다.
        3. 워크노드 다이어그램 안에서 다음을 입력할 수 있다.
          - "워크노드 이름" 라벨과 "..." 버튼 : "..." 버튼 클릭시 입력을 받는 말풍선이 표시되고 말풍선 안에 저장 버튼을 누르면 다이어그램의 "워크노드 이름" 라벨 옆에 표시된다. "..." 버튼은 이후 수정할 때도 사용된다.
          - "대상 에이전트" 라벨과 "..." 버튼 : "..." 버튼 클릭시 입력을 받는 말풍선이 표시되고 말퐁선 안에는 사용자에게 할당된 에이전트가 텍스트 레이블 버튼으로 나열된다. 이 중 한개를 선택할 수 있다. "..." 버튼은 이후 수정할 때도 사용된다.
          - "작업 설명" 라벨과 "..." 버튼 : "워크노드 이름" 입력 편집과 동일하다. 다만 입력값이 길 수 있으므로 말풍선 세로를 두 배 크기로 표시한다.
          - "에이전트 질의" 라벨과 "..." 버튼 : "워크노드 이름" 입력 편집과 동일하다. 다만 입력값이 길 수 있으므로 말풍선 세로를 두 배 크기로 표시한다. "에이전트 질의" 필드는 실제로 수행을 할 수 있는 버튼을 하나 더 추가하고 "play(옆으로 세운 삼각형)" 버튼을 둔다. play 버튼을 클릭하면 질의가 던져지는에 이 엑션은 현재는 더미 버튼으로 둔다.
          - "파일 업로드" 라벨과 "..." 버튼 : 에이전트가 수행할때 부수적으로 필요할 수 있는 파일 업로드 기능
          - 워크노드의 오른쪽에도 + 버튼을 동그라미로 표시된다. + 버튼을 누르면 팝업메뉴가 발생하고, 네 개 중 하나를 선택할 수 있다.
            ...
            4. 종료
        4. 승인자 다이어그램
          - "승인자" 라벨과 "..." 버튼 : "..." 버튼 클릭시 users.role = 0, 2 인 사용자를 텍스트 레이블 버튼으로 말풍선에 표시하고 선택할 수 있다(1명)
          - 승인자 다이어그램의 오른쪽에도 + 버튼을 동그라미로 표시된다. + 버튼을 누르면 팝업메뉴가 발생하고, 다음을 선택할 수 있다.
            1. 다음 워크노드
      - 워크플로우 다이어그램과 연결선은 모두 vertical center 정렬한다.
      - S -> 시작, E -> 종료 로 레이블 교체
      - 시작, 종료를 제외한 모든 워크 노드(승인자 포함) 다이어그램에는 오른쪽 상단에 "X" 박스를 두고 삭제 할 수 있게 한다. "X" 박스의 테두리는 없앤다.
      - 연결선은 다음 워크노드로 향하는 화살표로 변경
      - 생성 버튼은 "워크플로우 명" 입력 텍스트 필드의 오른쪽에 배치한다.
      - 워크 노드의 영역을 클릭시 워크노드가 선택되게 표시한다. 선택된 워크노드 편집을 위해 "워크플로우 디자인" 패널 하단에 "작업 편집" 패널을 세로 비율 50%(default)로 조정하고 마우스 드래그를 통해 조절할 수 있다
        - 워크노드 편집 패널은 워크플로우가 선택될때만 가변적으로 생성된다. 
        - 워크 플로우 외 다른 영역을 마우스 클릭하면 워크노드 편집 패널은 사라진다.
        - 워크노드 편집 패널의 표시 명을 변경한다 -> 작업 편집, PLAN문서에서 "워크노드" 라는 표현은 일관성을 유지하도록 "작업노드" 로 변경한다.
        - 작업 편집은 다음을 편집할 수 있다. 
          - 작업명
          - 대상 에이전트
          - 작업 설명
          - 에이전트 질의 -> 에이전트 명령으로 표현을 변경한다.
          - 파일 업로드
