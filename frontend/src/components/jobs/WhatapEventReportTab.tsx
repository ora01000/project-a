...
interface WhatapEventReportTabProps {
  active: boolean;
  onCopyToNote: (content: string, noteName?: string) => Promise<void>;
}

export function WhatapEventReportTab({ active, onCopyToNote }: WhatapEventReportTabProps) {
  ...
  const loadJobs = useCallback(async () => {
    ...
    try {
      const params = new URLSearchParams({
        ...
        job_type: String(JOB_TYPE_WHATAP),
      });
      const response = await fetch(`/api/jobs?${params.toString()}`);
      if (!response.ok) {
        throw new Error(await parseError(response, "Whatap 이벤트 리포트를 불러오지 못했습니다."));
      }
      ...
      setJobs([]);
      setSelectedIdx(null);
      setError(err instanceof Error ? err.message : "Whatap 이벤트 리포트를 불러오지 못했습니다.");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleCopyToNote = async () => {
    ...
    setError(null);
    try {
      const noteName = `${selectedJob.srnum} Whatap 리포트`.slice(0, 50);
