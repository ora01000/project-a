...
export function AgentAssignmentPage({ onClose }: AgentAssignmentPageProps) {
  ...
  const assignAllAgentsToUser = (userIdx: number) => {
    assignAgents(userIdx, allAgentIds);
  };
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-400">
              ...
              드래그하세요. 각 행의 모두 할당으로 왼쪽 목록 전체를 한 번에 할당할 수 있습니다.
              할당된 에이전트 레이블을 클릭하면 해제됩니다.
            </p>
          </div>
                    ...
                    {drafts.map((draft) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="whitespace-nowrap border-b border-slate-800 px-3 py-2 align-top">
                            <button
                              type="button"
                              disabled={agents.length === 0}
                              onClick={() => assignAllAgentsToUser(draft.idx)}
                              className="rounded border border-sky-700 bg-sky-950/40 px-2.5 py-1 text-[11px] font-medium text-sky-100 hover:bg-sky-900/50 disabled:cursor-not-allowed disabled:opacity-40"
                            >
                              모두 할당
                            </button>
                          </td>
