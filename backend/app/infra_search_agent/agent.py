"""Standalone INFRA_SEARCH_AGENT — inventory SQLite search via inventory MCP."""
...
from backend.app.config import MCPServerConfig, _env_setting, resolve_agent_runtime_mode
...
def _http_llm_api_key() -> str:
    return _env_setting(STATIC_CONFIG.http_llm_api_key_env)
...
def _build_llm(runtime_mode: str) -> ChatOpenAI:
    ...
    if runtime_mode == "http":
        api_key = _http_llm_api_key()
        if not api_key:
            raise RuntimeError(
                f"http 모드 INFRA_SEARCH_AGENT 는 {STATIC_CONFIG.http_llm_api_key_env} 가 필요합니다."
            )
...
class InfraSearchAgentService:
    """In-process LangGraph agent bound to the inventory MCP server."""
    ...
    async def initialize(self, runtime_mode: str | None = None) -> None:
        ...
        if not tools:
            prompt = (
                f"{prompt}\n\n"
                f"The inventory MCP server ({STATIC_CONFIG.mcp_server_key} @ {mcp_url}) "
                "is not connected yet. Inform the user that inventory search is unavailable."
            )
        ...
        logger.info(
            "INFRA_SEARCH_AGENT initialized mode=%s mcp_url=%s mcp_status=%s tools=%s tool_names=%s",
            mode,
            mcp_url,
            self._mcp_status,
            len(tools),
...
infra_search_agent_service = InfraSearchAgentService()
